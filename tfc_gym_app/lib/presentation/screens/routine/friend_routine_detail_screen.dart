...
class _FriendRoutineDetailScreenState extends State<FriendRoutineDetailScreen> {
  ...
  void initState() {
    super.initState();
    // NO autoseleccionamos ningún día para empezar; el usuario elige.
  }
  ...
  void _onDaySelected(int index, List<dynamic> splitDays) {
    setState(() {
      // Toggle: si era el mismo índice lo deseleccionamos
    ...
    // Solo carga ejercicios cuando hay día seleccionado
  ...
  Widget build(BuildContext context) {
    ...
    splitDays.sort(
        (a, b) => (a.dayName ?? 0).compareTo(b.dayName ?? 0)); // Lunes-Domingo
